  ...
  describe("notes", function() {
    ...
    it("should not update note if invalid _id", function() {
      expect(() => {
        Meteor.server.method_handlers["notes.update"].apply({
            userId: noteOne.userId
        }, [{_id: 'testid', title: 'New test title'}]);
      }).toThrow();
    });
    ...
    it('should not return notes for none existing user', function () {
        const res = Meteor.server.publish_handlers.notes.apply({userId: "noneExistin"});
        const notes = res.fetch();
